...
function left($id){   
	...
	$query="select up_id from menu_products where id = (SELECT up_id from menu_products WHERE id='".$id."')";//子id也要刪
	$res=MDB2_query($mdb2 ,$query);
	$rows=$res->fetchRow();	
	
	$query="update menu_products set up_id='".$rows[0]."' where id='".$id."'";
	$res=MDB2_query($mdb2 ,$query);	
	//	$objResponse->assign("msg","innerHTML",$query);
...
function order($id,$type){   
	...
	//find sno
	$query="select sno from menu_products where id ='$id'";
	$res=MDB2_query($mdb2 ,$query);
	$rows=$res->fetchRow();
	...
	switch(strtoupper($type)){
		case "UP":
			if ($rows[0] > 0){
				$val=$rows[0]-1;
				$query="update menu_products set sno='$val' where id='".$id."'";
				$res=MDB2_query($mdb2 ,$query);			
				$msg=$id."排序已向前";
			}else{
				$msg=$id."已到最前";
...
function delLoop($id,$mdb2){//子項也要刪除
	$query="DELETE FROM menu_products WHERE id='$id'"; //刪了自己這一筆
	$res=MDB2_query($mdb2 ,$query);
	
	$query="SELECT id from menu_products WHERE up_id='$id'";//子id也要刪
